...
def train_model(config):
    ...
    print('\nInitializing HTNetEnhanced model with Diagonal Micro-Attention and ROI modules...')
    model = HTNetEnhanced(
        image_size=config.image_size,
    ...
    print(f'Diagonal Micro-Attention: {config.use_diagonal_attn}')
    print(f'ROI Module: {config.use_roi}')
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if val_metrics['accuracy'] > best_val_acc:
            ...
            save_path = os.path.join(config.save_dir, 'best_model_enhanced.pth')
            torch.save({
                'epoch': epoch,
        ...
        if epoch % config.save_interval == 0:
            checkpoint_path = os.path.join(config.save_dir, f'checkpoint_enhanced_epoch_{epoch}.pth')
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
    ...
    history_path = os.path.join(config.log_dir, 'training_history_enhanced.json')
...
def main():
    parser = argparse.ArgumentParser(description='Train HTNetEnhanced for Facial Palsy Recognition with Diagonal Micro-Attention and ROI')
    ...
    parser.add_argument('--use_diagonal_attn', type=bool, default=True,
                       help='Use diagonal micro-attention for asymmetry detection')
    parser.add_argument('--use_roi', type=bool, default=True,
                       help='Use ROI attention module for facial region focus')
    ...
    parser.add_argument('--save_dir', type=str, default='./checkpoints/enhanced',
                       help='Directory to save model checkpoints')
    parser.add_argument('--log_dir', type=str, default='./logs/enhanced',
    ...
    print('\n' + '='*60)
    print('HTNetEnhanced for Facial Palsy Recognition')
    print('With Diagonal Micro-Attention and ROI Module')
    print('='*60)
    print('\nConfiguration:')
